# KamiYomu Crawler Agent — mangapark.net

A dedicated crawler agent for accessing public manga data from `mangapark.net`. Built on `KamiYomu.CrawlerAgents.Core`, this agent enables efficient search, metadata extraction, and integration with the KamiYomu platform.

## Features
- Search public `mangapark.net` content (titles, authors, tags)
...
The package is published as `KamiYomu.CrawlerAgents.MangaPark`.
...
3. Use the platform search or API to query `mangapark.net` data. Example usage patterns:
...
- Navigate to `/src/KamiYomu.CrawlerAgents.MangaPark/`
- Double-click `KamiYomu.CrawlerAgents.MangaPark.sln`
...
- VS Code: Terminal > `dotnet build src/KamiYomu.CrawlerAgents.MangaPark/KamiYomu.CrawlerAgents.MangaPark.sln`
