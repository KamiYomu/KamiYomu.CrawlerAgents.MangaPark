...
Dictionary<string, object> options = new()
{
    ...
    { "Mirror", "https://comicpark.org" }
};
ICrawlerAgent crawler = new MangaParkCrawlerAgent(options);
List<(string Method, bool Success, string Message)> results = [];

PagedResult<Manga> mangaResult = await crawler.SearchAsync("One Piece", new PaginationOptions(1, 30), CancellationToken.None);
#endregion
